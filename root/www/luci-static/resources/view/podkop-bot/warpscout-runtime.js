...
function timeoutText(tg) {
	var ms=parseInt(tg&&tg.latency_ms||0,10), sec=ms>0?(ms/1000).toFixed(1):'5.0';
	return _('TIMEOUT')+' · '+((tg&&tg.reason==='connect_timeout')?_('connect timeout'):_('request timeout'))+' · '+sec+' s';
}
	...
	render: function(data) {
		...
		return E('div', {}, [
			...
			E('div', { 'class':'cbi-section pb-card', 'style':'max-width:820px;' }, [
				...
				row(_('WARP tunnel'), rt&&rt.running ? dot('green',_('OK')+(rt.pid?(' · PID '+rt.pid):'')+(rt.rss_mb!=null?(' · RSS '+rt.rss_mb+' MB') : '')) : dot('grey',rt&&rt.state||_('stopped'))),
				...
				(tg.error ? row(_('Telegram detail'), E('span', {}, tg.error)) : E('span', {})),
